defmodule Matrex.Loops.Sync do
  ...
  def init({transport, :http}, req, opts) do
    conn = CowboyConn.conn(req, transport)
  ...
  defp handle(conn) do
    case parse_args(conn) do
      {:ok, %{timeout: _} = args} -> loop(conn, args)
      {:ok, args} -> respond(conn, args)
      {:error, error} ->
        conn
          |> json_error(error)
          |> Conn.halt
  ...
  defp parse_args(conn) do
    ...
    with {:ok, args} <- optional(:filter, params, args, type: :string, post: &parse_filter/1),
         ...
         {:ok, args} <- optional(:timeout, params, args, type: :string, post: &parse_timeout/1)
    do
      {:ok, args}
    end
  end
